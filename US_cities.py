...
for i in range(1,315,1):
    ...
    try:
        ...
        try:
            city_summary=wikipedia.summary(city + "," + state)
            city_summary_list.append(city_summary)
        except PageError:
            try:
                city=driver.find_element_by_xpath(city_xpath).click()
                para_1_xpath='//*[@id="mw-content-text"]/div/p[1]'
                para_2_xpath='//*[@id="mw-content-text"]/div/p[2]'
                para_1=driver.find_element_by_xpath(para_1_xpath).text
                para_2=driver.find_element_by_xpath(para_2_xpath).text
                city_summary=para_1+ " " + para_2
                city_summary_list.append(city_summary)
                driver.get(url)
            except:
                continue
...
"""Dataframe to CSV""" 
df_03.to_csv(r'USA cities.csv', index=False)
